...
describe("shopify.app.toml", () => {
  ...
  it("keeps uninstall cleanup on the deployed app URL for localhost dev compatibility", () => {
    assert.match(
      appConfig,
      /topics = \["app\/uninstalled"\]\s+uri = "https:\/\/aura-historia\.com\/webhooks\/app\/uninstalled"/,
    );
  });

  it("allows the Shopify auth callback URLs used by the app", () => {
    assert.ok(appConfig.includes("https://aura-historia.com/auth/callback"));
    assert.ok(
      appConfig.includes("https://stage.aura-historia.com/auth/callback"),
    );
  });
});
